...
- Recruitment minimums and a normalized Tier X vehicle requirement expression when a server member with Manage Members configures recruiting criteria. WoTSTATS does not store evaluated candidates or reports.
- For an enabled player-count topic, the selected text-channel ID and the prior topic needed to restore that channel when the feature is disabled or moved.
...
When the optional `/fstats` command is enabled and invoked, WoTSTATS first resolves the requested player through Wargaming, then sends Tomato.gg that Wargaming account ID and mapped realm to request 7-day, 30-day, 100-battle, and 1,000-battle recent public statistics. Tomato.gg also receives ordinary request metadata such as the WoTSTATS server's network address and request time. WoTSTATS does not send Tomato.gg a Discord ID or nickname, Wargaming nickname or access token, private economy field, guild/channel/message context, or another player's battle participants. The Tomato response is used for that command response and is not stored as local player history or raw provider data. Existing minimized command telemetry does not store the target or returned statistics.

When the optional `/stats` quadbox is enabled, every resolved `/stats` target—not only the invoking user's linked account—is sent to Tomato.gg as the public Wargaming account ID and mapped realm for one 1,000-battle recent-statistics projection. Tomato.gg also receives ordinary request metadata such as the WoTSTATS server's network address and request time. WoTSTATS does not send a Discord ID or nickname, Wargaming nickname or access token, private economy field, guild/channel/message context, or another player's battle participants. The recent win rate and WN8 are rendered into a small in-memory thumbnail and the response and image bytes are discarded after delivery; no player history, raw response, image file, or player-specific cache is created. Overall win rate comes from Wargaming, overall WN8 is calculated locally from official public per-tank totals and the cached global ModXVM table, and vehicle-class bars are grouped locally from those official totals and the cached global vehicle catalog. When the quadbox is disabled, `/stats` makes no Tomato request.
...
When optional clan operations are enabled, `/clan_online` and `/boosts` use the invoking user's exact active linked account and encrypted Wargaming access token to request private clan information from Wargaming. Clan-online retrieves the caller's clan roster and the account IDs Wargaming reports as online. WoTSTATS does not map those account IDs to Discord users, mention members, or retain the roster. Boosts retrieves the clan's reserve inventory and active status. Manual reserve activation is available only to supported official clan roles, requires an explicit confirmation, and sends Wargaming the reserve type and level using the caller's token. WoTSTATS stores a bounded non-secret request/audit record for confirmation, duplicate prevention, outcome, and investigation; it never stores the token or raw provider response in that record. Automatic reserve activation is not included.

When optional `/recruit` is enabled, WoTSTATS resolves the requested public player through Wargaming and sends Tomato.gg that Wargaming account ID and mapped realm to request public overall and 1,000-battle recent statistics. Tomato.gg also receives ordinary request metadata such as the WoTSTATS server's network address and request time. WoTSTATS does not send Tomato.gg a Discord ID, Wargaming nickname or access token, configured benchmark, tank requirement, guild/channel/message context, or another player's battle participants. Official public Wargaming data supplies account details, clan history, and vehicle totals; the configured criteria, Tier X inventory, and required-tank expression are evaluated in memory. The provider responses and rendered quadbox are discarded after delivery. No candidate, report, provider response, player history, or image is stored.

When an administrator enables `/pchan`, WoTSTATS periodically requests the public online-player total for that server's selected realm from Wargaming and updates the configured Discord text-channel topic. The stored prior topic is used only to avoid erasing unrelated administrator text when the feature is disabled or moved. Topic changes are audited without retaining unrelated message content.
...
Clan-sensitive `/clan_online`, `/boosts`, and `/recruit` output also follows the server's WoTSTATS classification. In a Public/community server it is ephemeral. In a Clan/private server a successful report may be visible in the channel. Configuration, confirmation controls, errors, and reserve-activation previews remain ephemeral.
...
| Recruitment requirements and player-count channel settings | Part of server configuration: retained through the same 30-day rejoin window, then deleted. Stored requirements contain no evaluated player or report. A saved prior channel topic is bounded Discord configuration content and is removed with the binding. |
| Manual reserve activation request/audit metadata | Non-secret confirmation requests expire within minutes. Outcome and minimized audit metadata may be retained with other consequential security events; direct identifiers are subject to the deletion process below. No token or provider response is retained. |
...
- Wargaming's official API, to authorize linked accounts, retrieve requested World of Tanks information, retrieve enabled private clan-online/reserve data for the invoking linked user, perform an explicitly confirmed manual clan-reserve activation, and supply public realm totals for an enabled player-count topic;
...
- Tomato.gg, when optional `/fstats` is enabled and invoked, to retrieve recent public statistics using the requested Wargaming account ID and realm; when the optional `/stats` quadbox is enabled, to retrieve and transiently render the resolved target's public 1,000-battle recent win rate and WN8; when optional linked-account `/marks` comparison is enabled, to retrieve and transiently project the invoking linked account's selected lifetime tank statistics from an overall response; when optional `/chart` is enabled, to transiently render the invoking linked account's selected lifetime progression metric over total battles; and when optional `/recruit` is enabled, to transiently evaluate the requested public player's overall and 1,000-battle recent statistics against server-configured criteria; and
- Cloudflare, when the approved public callback hostname is enabled, to route and protect the web callback.

Those providers process information under their own terms and privacy notices. Tomato.gg's policy is available at [tomato.gg/privacy-policy](https://tomato.gg/privacy-policy). Other optional statistics sites, Top.gg posting, automatic clan-reserve activation, and unrelated premium/test features are not enabled. If a later feature changes the data shared or collected, this notice must be updated before that feature is enabled.
